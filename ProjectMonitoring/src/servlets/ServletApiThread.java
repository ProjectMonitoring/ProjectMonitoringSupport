...
import models.threadDAO;

public class ServletApiThread extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<thread> allThreads = dao.selectAllThread();
		...
		String conJSON = gson.toJson(allThreads);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String response = req.getParameter("thread");
		
		thread newThread = gson.fromJson(response, thread.class);
		...
		try
		{
			...
			inserted = dao.insertThread(newThread);
		...
		if(inserted)
		{
			writer.write("new thread inserted");
		}
		else
		{
			writer.write("error");
	...
	protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		thread updateThread = gson.fromJson(response, thread.class);
		...
		try
		{
			...
			updated = dao.updateThread(updateThread, updateThread.getId());
		} catch (SQLException e) {
			e.printStackTrace();
		}
		...
		if(updated)
		{
			writer.write("Thread: " + updateThread.getTitle() + " updated");
		}
		else
		{
			writer.write("error");
	...
	protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("Thread to delete: " + id);
		
		boolean deleted = false;
		
		try
		{
			deleted = dao.deleteThread(id);
		} catch (SQLException e) {
			e.printStackTrace();
		}
